...
import logic.GetSeafoodMealsUseCase.Companion.SEAFOOD_KEYWORDS
...
class GetSeafoodMealsUseCaseTest() {
    ...
    @Test
    fun `should return only sea food meals when one of the keywords found in ingredients`() {
        ...
        //Then
        result.map { meal -> meal.name }.shouldContain("sea food meal")
    }

    @Test
    fun `should return empty list when there are no meals found in list`() {

        //Given
        every { datasource.getAllMeals() } returns getMealsItem().map { meal ->
            meal.copy(ingredients = meal.ingredients - SEAFOOD_KEYWORDS.toSet())
        }
        //When
        val result = getSeafoodMealsUseCase.getSeafoodMeals()
        //then
